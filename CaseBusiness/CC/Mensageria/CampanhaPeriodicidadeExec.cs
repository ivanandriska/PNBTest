...
{
    public class CampanhaPeriodicidadeExec : BusinessBase
    {
        #region MemoryCache
        private const String kCacheKey = "CaseBusiness_CC_Mensageria_CampanhaPeriodicidadeExecs_DataTable";
        private const Int16 kCache_ABSOLUTEEXPIRATION_MINUTES = 1440;  // 60 minutos * 24 horas
        #endregion MemoryCache

        #region Atributos
        private Int32 _idCampanhaPeriodicidadeExecs = Int32.MinValue;
        ...
        #region Propriedades
        public Int32 IdCampanhaPeriodicidadeExecs
        {
            get { return _idCampanhaPeriodicidadeExecs; }
            set { _idCampanhaPeriodicidadeExecs = value; }
        }
        ...
        public CampanhaPeriodicidadeExec(Int32 idUsuarioManutencao, Boolean modoEntidadeOnly)
        {
        ...
        public CampanhaPeriodicidadeExec(Int32 idUsuarioManutencao)
        {
            UsuarioManutencao.ID = idUsuarioManutencao;
        ...
        public CampanhaPeriodicidadeExec(Int32 idUsuarioManutencao, CaseBusiness.Framework.BancoDados.Transacao transacao)
        {
            UsuarioManutencao.ID = idUsuarioManutencao;
        ...
        //    try
        //    {
        //        if (ModoEntidadeOnly) { throw new ApplicationException("Esta Instância da classe CampanhaPeriodicidadeExecs está operando em Modo Entidade Only"); }
        ...
        /// <summary>
        /// Excluir uma Periodicidade de Execução
        /// </summary>        
        /// <param name="idCampanha">Id Campanha</param>
        public void Excluir(Int32 idCampanha)
        {
            try
            {
                ...
                acessoDadosBase.ExecuteNonQuerySemRetorno(CommandType.StoredProcedure, "prCMPPRDEX_DEL");
            }
            catch (Exception ex)
            {
                CaseBusiness.Framework.Log.Log.Logar(CaseBusiness.Framework.TipoLog.Erro, ex.Message, ex.StackTrace, "", "", DateTime.Now, CaseBusiness.Framework.Configuracao.Configuracao.Aplicacao, CaseBusiness.Framework.Tela.Nenhum, 0);
        ...
        private void RenomearColunas(ref DataTable dt)
        {
            ...
            if (dt.Columns.Contains("CMPPRDEX_TP_FREQ_EXEC")) { dt.Columns["CMPPRDEX_TP_FREQ_EXEC"].ColumnName = "TipoFrequenciaExec"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_DOM")) { dt.Columns["CMPPRDEX_FL_DOM"].ColumnName = "FlagDomingo"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_SEG")) { dt.Columns["CMPPRDEX_FL_SEG"].ColumnName = "FlagSegunda"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_TER")) { dt.Columns["CMPPRDEX_FL_TER"].ColumnName = "FlagTerca"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_QUA")) { dt.Columns["CMPPRDEX_FL_QUA"].ColumnName = "FlagQuarta"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_QUI")) { dt.Columns["CMPPRDEX_FL_QUI"].ColumnName = "FlagQuinta"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_SEX")) { dt.Columns["CMPPRDEX_FL_SEX"].ColumnName = "FlagSexta"; }
            if (dt.Columns.Contains("CMPPRDEX_FL_SAB")) { dt.Columns["CMPPRDEX_FL_SAB"].ColumnName = "FlagSabado"; }
            if (dt.Columns.Contains("CMPPRDEX_QT_FREQ_SEM")) { dt.Columns["CMPPRDEX_QT_FREQ_SEM"].ColumnName = "QuantFrequenciaSemanal"; }
            if (dt.Columns.Contains("CMPPRDEX_TP_FREQ_EXEC_MEN")) { dt.Columns["CMPPRDEX_TP_FREQ_EXEC_MEN"].ColumnName = "TipoFrequenciaExecMensal"; }
            if (dt.Columns.Contains("CMPPRDEX_QT_FREQ_MEN")) { dt.Columns["CMPPRDEX_QT_FREQ_MEN"].ColumnName = "QuantFrequenciaMensal"; }
            if (dt.Columns.Contains("CMPPRDEX_TP_DIA_MEN")) { dt.Columns["CMPPRDEX_TP_DIA_MEN"].ColumnName = "TipoDiaMensal"; }
            if (dt.Columns.Contains("CMPPRDEX_ORD_MEN")) { dt.Columns["CMPPRDEX_ORD_MEN"].ColumnName = "OrdemMensal"; }
            if (dt.Columns.Contains("CMPPRDEX_ORD_DIA_MEN")) { dt.Columns["CMPPRDEX_ORD_DIA_MEN"].ColumnName = "OrdemDiaMensal"; }
